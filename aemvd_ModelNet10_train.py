from MVD1 import AEMVD as aemvd
from PIL import Image
import numpy as np
import os, fnmatch
import time
import keras

_start_time = time.time()


def tic():
    global _start_time
    _start_time = time.time()


def tac():
    t_sec = round(time.time() - _start_time)
    (t_min, t_sec) = divmod(t_sec, 60)
    (t_hour, t_min) = divmod(t_min, 60)
    print('Time passed: {}hour:{}min:{}sec'.format(t_hour, t_min, t_sec))


def length_of_dataset(path, type):
    class_list = os.listdir(path)
    i = 0
    for c in class_list:
        obj_dir_path = path + '/' + c + '/' + type
        # print (obj_dir_path)
        obj_list = os.listdir(obj_dir_path)
        i += len(obj_list)
    print (i)
    return i


def load_dataset(path, type):
    obj_num = length_of_dataset(path, type)
    x = np.zeros((obj_num, 128, 128, 12))
    y = np.zeros((obj_num, 64, 64, 12))
    # first = True
    class_list = os.listdir(path)
    i = 0
    for c in class_list:
        obj_dir_path = path + '/' + c + '/' + type
        # print (obj_dir_path)
        obj_list = os.listdir(obj_dir_path)
        for obj in obj_list:
            obj_view_path = obj_dir_path + '/' + obj
            # print (obj_view_path)
            # file_list = fnmatch.filter(os.listdir(obj_view_path), 'view_*')
            for v_i in range(0, 12):
                x_file_path = obj_view_path + '/view_' + str(v_i) + '.png'
                y_file_path = obj_view_path + '/depth_' + str(v_i) + '.png'
                # print (file_path)
                img = Image.open(x_file_path).convert('L')
                img.load()
                img = img.resize((x.shape[2], x.shape[1]), Image.ANTIALIAS)
                data = np.asarray(img, dtype="int32")
                x[i, :, :, v_i] = data

                img = Image.open(y_file_path).convert('L')
                img.load()
                img = img.resize((y.shape[2], y.shape[1]), Image.ANTIALIAS)
                data = np.asarray(img, dtype="int32")
                y[i, :, :, v_i] = data

            i += 1
            print(i)

    x[x >= 255] = 0
    return x, y


def load_input_dataset(path, type):
    obj_num = length_of_dataset(path, type)
    x = np.zeros((obj_num, 128, 128, 12))
    # first = True
    class_list = os.listdir(path)
    i = 0
    for c in class_list:
        obj_dir_path = path + '/' + c + '/' + type
        # print (obj_dir_path)
        obj_list = os.listdir(obj_dir_path)
        for obj in obj_list:
            obj_view_path = obj_dir_path + '/' + obj
            # print (obj_view_path)
            file_list = fnmatch.filter(os.listdir(obj_view_path), 'view_*')
            v_i = 0
            for file in file_list:
                file_path = obj_view_path + '/' + file
                # print (file_path)
                img = Image.open(file_path).convert('L')
                img.load()
                img = img.resize((x.shape[2], x.shape[1]), Image.ANTIALIAS)
                data = np.asarray(img, dtype="int32")
                x[i, :, :, v_i] = data
                v_i += 1

            i += 1
            print(i)

    x[x >= 255] = 0
    return x


def load_output_dataset(path, type):
    obj_num = length_of_dataset(path, type)
    y = np.zeros((obj_num, 64, 64, 12))
    class_list = os.listdir(path)
    i = 0
    for c in class_list:
        obj_dir_path = path + '/' + c + '/' + type
        # print (obj_dir_path)
        obj_list = os.listdir(obj_dir_path)
        for obj in obj_list:
            obj_view_path = obj_dir_path + '/' + obj
            # print (obj_view_path)
            file_list = fnmatch.filter(os.listdir(obj_view_path), 'depth_*')
            v_i = 0
            for file in file_list:
                file_path = obj_view_path + '/' + file
                # print (file_path)
                img = Image.open(file_path).convert('L')
                img.load()
                img = img.resize((y.shape[2], y.shape[1]), Image.ANTIALIAS)
                data = np.asarray(img, dtype="int32")
                y[i, :, :, v_i] = data
                v_i += 1

            i += 1
            print(i)
    return y


def normal(X):
    min = np.min(np.min(X))
    max = np.max(np.max(X))
    X = (X - min) / max
    return X


# Initialize Dataset
# path = "E:\Dataset\ModelNet10\ModelNet10_multiview"
# x_train, y_train = load_dataset(path, 'train')
# x_test, y_test = load_dataset(path, 'test')
# x_train = normal(x_train)
# x_test = normal(x_test)
# y_train = normal(y_train)
# y_test = normal(y_test)

# np.save('x_train_w.npy', x_train)
# np.save('x_test_w.npy', x_test)
# np.save('y_train.npy', y_train)
# np.save('y_test.npy', y_test)

x_train=np.load('x_train_w.npy')
print('x train loaded')
x_test=np.load('x_test_w.npy')
print('x test loaded')

y_train=np.load('y_train.npy')
print('y train loaded')
y_test=np.load('y_test.npy')
print('y test loaded')

# Initialize the aemvd network
model_name = 'model_aemvd_modelnet10_2'
first_run = True

if first_run:
    # create model
    model = aemvd.init_model()
    model = aemvd.compile(model)
else:
    model = aemvd.load_model_and_weight('model/' + model_name)
    model = aemvd.compile(model)

min_err = 1000
n_epoch_save = 10
counter = 0
epoch_co = 0
tic()
for i in range(epoch_co, 3000):
    model = aemvd.train(model, x_train, y_train, n_epoch=1, batch_size=32)
    err = aemvd.get_error_rate(model, x_test, y_test)
    tac()
    if err < min_err:
        aemvd.save_model_and_weight(model, 'model/' + model_name)
        min_err = err
    else:
        print("epoch: " + str(i) + "   error:" + str(err))
    if counter >= n_epoch_save:
        aemvd.save_model_and_weight(model, 'model/' + model_name + "_epoch_" + str(i))
        counter = 0
    counter += 1

tac()
